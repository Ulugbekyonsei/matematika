...
const Figures = (() => {
  ...
  // A crate takes an emoji (repeated to fill the box) or ready-made markup —
  // there is no pomegranate emoji, so anor is drawn as SVG glyphs.
  function twoCrates(fruitA, labelA, fruitB, labelB) {
    const crate = (fruit, label) => `
      <div class="crate">
        <div class="crate-box">${fruit.startsWith('<') ? fruit : fruit.repeat(6)}</div>
        <div class="crate-label">${label}</div>
      </div>`;
    return `<div class="crate-row">${crate(fruitA, labelA)}${crate(fruitB, labelB)}</div>`;
  }

  /* Anor — round red fruit with its little crown. */
  function pomegranates(n = 6) {
    const one = `<svg class="pom-glyph" viewBox="0 0 26 28" width="23" height="25"
        xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path d="M9.5 7.5 L10.5 3.5 L13 6 L15.5 3.5 L16.5 7.5 Z" fill="#8f2433"/>
      <circle cx="13" cy="17" r="9.4" fill="#e04a52"/>
      <path d="M5.6 14 a9.4 9.4 0 0 1 5 -5.6" fill="none" stroke="#f3969b"
  ...
  return {
    ...
    fourRectangles, sideLabeledShape, pomegranates
  };
