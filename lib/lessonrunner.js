...
const LessonRunner = (() => {
  ...
  function renderInfo(step, body, actions) {
    ...
    body.innerHTML = `
      ...
      ${blocks ? `<div class="namuna-grid">${blocks}</div>` : ''}
      ${step.note ? `<div class="namuna-note">${step.note}</div>` : ''}`;
  ...
  function renderChoice(step, body) {
    ...
    body.innerHTML = `
      ...
      ${step.figure ? `<div class="ex-figure">${step.figure}</div>` : ''}
      ${step.expr ? `<div class="question-line"><span class="question">${step.expr}</span></div>` : ''}
