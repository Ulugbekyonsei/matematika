...
const Keypad = (() => {
   ...
   * onSubmit(value:number) fires on ✓ or once `maxDigits` digits are entered.
   *
   * `maxDigits` is set per exercise from its widest answer, not per question,
   * so it never hints at the length of the answer in front of her.
   */
  function mount(host, { onSubmit, onChange, maxDigits = MAX_DIGITS }) {
    let value = '';
    let locked = false;
    ...
    function press(k) {
      ...
      if (value.length === maxDigits) submit();
    }
